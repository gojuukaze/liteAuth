...
.. py:data:: NOTIFICATION_BACKEND
   ...
   * **FeiShu**
     ...
     你需要开通飞书机器人，步骤如下：
       ...
       #. 在"应用功能"中启动机器人功能
       #. 在"权限管理"中给予权限： ``用户 - 通过手机号或者邮箱获取用户ID`` , ``消息 - 给多个用户批量发消息`` , ``消息 - 以应用的身份发消息``
       #. 在“版本管理与发布”中创建版本并发布。（需要联系管理员审核通过）
...
.. note::
   ...
   所有支持的数据库，以及更多说明参考： https://docs.djangoproject.com/en/3.1/ref/databases/
